...
def recognize_faces_in_persons(frame, person_bboxes, face_tracker: FaceTracker):
    ...
    for face_box in face_boxes:
        face_id = tuple(face_box)  # Use box coordinates as a unique ID
        x1, y1, x2, y2 = face_box
        cropped_face = frame[y1:y2, x1:x2]
        score, name = face_rec(cropped_face)

        detection = f"SIETIAN ({name})" if name else "UNKNOWN"
        face_tracker.update_face(face_id, face_box, detection, current_time)

    # Update face states and prune lost faces
    face_tracker.update_states(current_time)

    # Draw bounding boxes and labels
    for face_id, data in face_tracker.tracker.items():
        x1, y1, x2, y2 = data["box"]
        color = (0, 255, 0) if data["state"] == "SIETIAN" else (0, 0, 255)
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
    ...
    states = ["UNDETERMINED"] * len(person_bboxes)
    for idx, person_box in enumerate(person_bboxes):
        for face_id, face_data in face_tracker.tracker.items():
            if is_face_in_person_box(face_data["box"], person_box):
                states[idx] = face_data["state"]
                break
