...
detector = SCRFD(
    model_file=os.path.join(
        BASE_DIR, "face_detection", "scrfd", "weights", "scrfd_10g_bnkps.onnx"
    )
)
...
def load_config(file_name):
    with open(file_name, "r") as stream:
        try:
            ...
            print(exc)
# Initialize tracker outside of the function to maintain state across frames
file_name = os.path.join(
    BASE_DIR, "face_tracking", "config", "config_tracking.yaml"
)
...
def process_tracking(frame, tracker, args, fps):
    ...
    return {
        ...
        "detection_bboxes": bboxes,  # Detected face boxes
        "detection_landmarks": landmarks,  # Detected landmarks
        "tracking_ids": tracking_ids,  # Tracking IDs
        "tracking_bboxes": tracking_bboxes  # Tracking bounding boxes
    }
...
def recognize_face(frame, person_boxes):
    ...
    for i, (face_box, face_id) in enumerate(zip(tracking_data["tracking_bboxes"],
        ...
        for j, (det_box, landmark) in enumerate(zip(tracking_data["detection_bboxes"],
            ...
            if mapping_bbox(face_box, det_box) > 0.9:
                ...
                # Remove processed detection from further consideration
    ...
    frame_id += 1  # Increment frame ID after processing the frame
