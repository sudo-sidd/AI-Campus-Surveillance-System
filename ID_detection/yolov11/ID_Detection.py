...
def detect_id_card(frame):
    ...
    for person_id, data in updated_tracked_persons.items():
        ...
        if current_time - start_time >= 3:  # Check if tracked for at least 3 seconds
            if id_card:
                association_status.append(f"Person {person_id}: Wearing ID Card ({id_card})")
            else:
                association_status.append(f"Person {person_id}: No ID Card associated")
        else:
            association_status.append(f"Person {person_id}: Waiting for confirmation...")
