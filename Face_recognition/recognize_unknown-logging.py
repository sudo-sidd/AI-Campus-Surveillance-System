...
def recognize():
    ...
    while not stop_event.is_set():
            ...
            for j in range(len(detection_bboxes)):
                ...
                if mapping_score > 0.9:
                    ...
                    if name is not None:
                        if score < 0.5:
                            caption = "UNKNOWN"
                            face_id = tracking_ids[i]

                            # If this face is newly unknown, add it to the timer
                            if face_id not in unknown_face_timers:
                                unknown_face_timers[face_id] = current_time

                            # If the face has been unknown for more than 3 seconds, store it
                            if current_time - unknown_face_timers[face_id] >= 3:
                                store_unknown_image(raw_image, tracking_bboxes[i], caption)
                                # Reset the timer after storing
                                unknown_face_timers[face_id] = current_time
                        else:
                            caption = f"Identified : {name}:{score:.2f}"
                            # If this face was previously unknown but now identified, remove it from timers
                            if tracking_ids[i] in unknown_face_timers:
                                del unknown_face_timers[tracking_ids[i]]
        ...
        # Clean up timers for faces that are no longer tracked
        unknown_face_timers = {face_id: start_time
                               for face_id, start_time in unknown_face_timers.items()
                               if face_id in tracking_ids}
...
def store_unknown_image(image, bbox, caption):
    """
    Store image of unknown person with metadata.

    Args:
        image (numpy.ndarray): The full frame image.
        bbox (list): Bounding box coordinates [x1, y1, x2, y2].
        caption (str): Caption for the image (e.g., "UNKNOWN").
    """
    ...
    filename = f"unknown_{timestamp}_{unique_id}.jpg"
    ...
    cv2.imwrite(str(unknown_dir / filename), face_img)
    ...
    metadata_filename = f"{filename.rsplit('.', 1)[0]}.json"
    with open(unknown_dir / metadata_filename, "w") as f:
